...
public class CharacterUtil {
    ...
    private static CharacterUtil sInstance;
    private final boolean mHasChinaCollator;

    protected CharacterUtil(boolean hasChinaCollator) {
    ...
    public static CharacterUtil getInstance() {
        synchronized (CharacterUtil.class) {
            ...
            for (Locale value : locale) {
                if (value.equals(Locale.CHINA) || value.equals(chinaAddition)) {
                    if (DEBUG) {
                        Log.d(TAG, "Self validation. Result: " + doSelfValidation());
                    }
                    sInstance = new CharacterUtil(true);
                    return sInstance;
                }
            }
            Log.w(TAG, "There is no Chinese collator, CharacterUtil is disabled");
            sInstance = new CharacterUtil(false);
            return sInstance;
        }
    }
...
// c. current character is space.
    ...
    private void addToken(@NonNull final StringBuilder sb, @NonNull final ArrayList<Token> tokens, final int tokenType) {
