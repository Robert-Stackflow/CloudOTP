package com.cloudchewie.ui.loadingdialog.view;
...
//        initAttr(context, attrs, defStyleAttr);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthSpecMode != MeasureSpec.AT_MOST && heightSpecMode != MeasureSpec.AT_MOST) {
            mWidth = Math.max(widthSpecSize, heightSpecSize);
        } else if (widthSpecMode == MeasureSpec.AT_MOST && heightSpecMode != MeasureSpec.AT_MOST) {
            mWidth = heightSpecSize;
        } else if (widthSpecMode != MeasureSpec.AT_MOST) {
            mWidth = widthSpecSize;
        } else {
            mWidth = SizeUtil.dp2px(mContext, 80);
        }
...
//    }
    ...
    private void drawDynamic(Canvas canvas) {
        if (progress < 100)
            progress += speed;
        //根据进度画圆弧
        canvas.drawArc(rectF, 235, (float) (360 * progress) / 100, false, mPaint);
    ...
    private void drawStatic(Canvas canvas) {
        ...
        canvas.drawLine(line1_start, line1_start,
                line1_start + (float) (2 * mWidth) / 5, line1_start + (float) 2 * mWidth / 5, mPaint);
        canvas.drawLine(line1_start + (float) 2 * mWidth / 5, line1_start,
                line1_start, line1_start + (float) 2 * mWidth / 5, mPaint);
    }
