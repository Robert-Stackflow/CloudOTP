...
import com.cloudchewie.ui.emoji.ImageLoader;
...
public class EmoticonsToolBarView extends RelativeLayout {
    ...
    public EmoticonsToolBarView(Context context, AttributeSet attrs) {
        ...
        mInflater.inflate(R.layout.widget_emoji_toolbar, this);
    ...
    public void addFixedToolItemView(@NonNull View view, boolean isRight) {
    ...
    protected View getCommonItemToolBtn() {
        return mInflater == null ? null : mInflater.inflate(R.layout.item_emoji_toolbar_button, null);
    }

    protected void initItemToolBtn(@NonNull View toolBtnView, int rec, final PageSetEntity pageSetEntity, OnClickListener onClickListener) {
        ImageView iv_icon = toolBtnView.findViewById(R.id.iv_icon);
        if (rec > 0) iv_icon.setImageResource(rec);
        ...
        toolBtnView.setOnClickListener(onClickListener != null ? onClickListener : view -> {
    ...
    protected View getToolBgBtn(@NonNull View parentView) {
    ...
    public void setToolBtnSelect(String uuid) {
        ...
        for (int i = 0; i < mToolBtnList.size(); i++) {
            ...
            if (object instanceof PageSetEntity && uuid.equals(((PageSetEntity) object).getUuid())) {
                mToolBtnList.get(i).setBackgroundColor(getResources().getColor(R.color.card_pressed_background));
                select = i;
            } else {
                mToolBtnList.get(i).setBackgroundResource(R.drawable.ic_light_fallback);
            }
        }
