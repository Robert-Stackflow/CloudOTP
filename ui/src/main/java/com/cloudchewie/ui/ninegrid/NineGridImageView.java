...
public class NineGridImageView<T> extends ViewGroup {
    ...
    public void setImagesData(List<T> lists, int spanType) {
        ...
        if (mImgDataList.size() > mMaxSize) {
            GridImageView childrenView = (GridImageView) getChildAt(mMaxSize - 1);
    ...
    public void deleteImage(int pos) {
        if (pos >= mImageViewList.size()) return;
     ...
     */
    @Nullable
    private ImageView getImageView(int count, final int position) {
        if (position < mImageViewList.size()) {
            return mImageViewList.get(position);
        } else {
            if (mAdapter != null) {
                ImageView imageView = (ImageView) mAdapter.generateImageView(count, position, getContext());
