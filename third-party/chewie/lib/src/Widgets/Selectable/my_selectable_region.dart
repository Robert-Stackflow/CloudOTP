...
/// with a [MySelectableRegion] widget enables selection within that subtree (for
...
///
/// A [MySelectableRegion] widget requires configuration; in particular specific
/// [selectionControls] must be provided.
///
/// The [SelectionArea] widget from the [material] library configures a
/// [MySelectableRegion] in a platform-specific manner (e.g. using a Material
/// toolbar on Android, a Cupertino toolbar on iOS), and it may therefore be
/// simpler to use that widget rather than using [MySelectableRegion] directly.
///
/// ## An overview of the selection system.
///
/// Every [Selectable] under the [MySelectableRegion] can be selected. They form a
/// selection tree structure to handle the selection.
///
/// The [MySelectableRegion] is a wrapper over [SelectionContainer]. It listens to
...
///
/// The [SelectionContainer] created by [MySelectableRegion] is the root node of a
...
/// a [MySelectableRegion], consider wrapping that group of widgets using
...
/// wrap part of the subtree with another [MySelectableRegion]. The selection of the
...
/// See also:
///
///  * [SelectionArea], which creates a [MySelectableRegion] with
...
class MySelectableRegion extends StatefulWidget {
  /// Create a new [MySelectableRegion] widget.
  ///
  ...
  ///
  /// By default, [MySelectableRegion]'s [TextMagnifierConfiguration] is disabled.
  /// For a version of [MySelectableRegion] that adapts automatically to the
  ...
  /// Returns the [MyContextMenuItem]s representing the buttons in this
  /// platform's default selection menu.
  ///
  /// For example, [MySelectableRegion] uses this to generate the default buttons
  /// for its context menu.
  ///
  /// See also:
  ///
  /// * [MySelectableRegionState.contextMenuButtonItems], which gives the
  ///   [MyContextMenuItem]s for a specific SelectableRegion.
  ...
  ///   take a list of [MyContextMenuItem]s with
  ///   [AdaptiveTextSelectionToolbar.buttonItems].
  /// * [AdaptiveTextSelectionToolbar.getAdaptiveButtons], which builds the button
  ///   Widgets for the current platform given [MyContextMenuItem]s.
  static List<MyContextMenuItem> getSelectableButtonItems({
  ...
  @override
  State<StatefulWidget> createState() => MySelectableRegionState();
}

/// State for a [MySelectableRegion].
class MySelectableRegionState extends State<MySelectableRegion>
  ...
  String? get selectedText => _selectable?.getSelectedContent()?.plainText;
  ...
  @override
  void didUpdateWidget(MySelectableRegion oldWidget) {
  ...
  /// See also:
  ///
  ///  * [contextMenuButtonItems], which provides the [MyContextMenuItem]s
  ...
  /// See also:
  ///
  /// * [MySelectableRegion.getSelectableButtonItems], which performs a similar role,
  ...
  ///   button Widgets for the current platform given [MyContextMenuItem]s.
  List<MyContextMenuItem> get contextMenuButtonItems {
    return MySelectableRegion.getSelectableButtonItems(
  ...
  List<MyContextMenuItem> get _textProcessingActionButtonItems {
    final List<MyContextMenuItem> buttonItems = <MyContextMenuItem>[];
    final SelectedContent? data = _selectable?.getSelectedContent();
    if (data == null) {
      return buttonItems;
    }
    ...
    for (final ProcessTextAction action in _processTextActions) {
      buttonItems.add(MyContextMenuItem(
        label: action.label,
        onPressed: () async {
...
class _SelectAllAction extends _NonOverrideAction<SelectAllTextIntent> {
  ...
  final MySelectableRegionState state;
...
class StaticSelectionContainerDelegate
  ...
  void didReceiveSelectionEventFor(
      {required Selectable selectable, bool? forEnd}) {
    switch (forEnd) {
      case true:
...
abstract class MultiSelectableSelectionContainerDelegate
  ...
  static int _compareScreenOrder(Selectable a, Selectable b) {
    final Rect rectA =
        MatrixUtils.transformRect(a.getTransformTo(null), _getBoundingBox(a));
    final Rect rectB =
        MatrixUtils.transformRect(b.getTransformTo(null), _getBoundingBox(b));
  ...
  SelectionGeometry getSelectionGeometry() {
    if (currentSelectionEndIndex == -1 ||
      ...
      return SelectionGeometry(
          status: SelectionStatus.none, hasContent: selectables.isNotEmpty);
    }
  ...
  @override
  int get contentLength => selectables.fold<int>(
      0, (int sum, Selectable selectable) => sum + selectable.contentLength);
...
class SelectionListener extends StatefulWidget {
  ...
  const SelectionListener(
      {super.key, required this.selectionNotifier, required this.child});
...
class _SelectionListenerState extends State<SelectionListener> {
  ...
  Widget build(BuildContext context) {
    return SelectionContainer(
        delegate: _selectionDelegate, child: widget.child);
  }
}
...
final class SelectionListenerNotifier extends ChangeNotifier {
  ...
  SelectionDetails get selection =>
      _selectionDelegate ??
      (throw Exception(
          'Selection client has not been registered to this notifier.'));
  ...
  void _registerSelectionListenerDelegate(
      _SelectionListenerDelegate selectionDelegate) {
    assert(
      !registered,
